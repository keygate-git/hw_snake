...
function move() {
    ...
    var showScore = document.getElementsByClassName('score')[0];
    showScore.innerHTML = 'Ваш текущий результат: ' + score + '<br>Скорость змейки: ' + (1 + parseInt(((200 / snake_speed) % 1) * 1000));
}
...
function createObstacle() {
    ...
    while (!obstacleCreated) { //пока препятствие не создали
        ...
        if (!obstacle_cell_classes.includes('snake-unit') && !obstacle_cell_classes.includes('food-unit') && !obstacle_cell_classes.includes('obstacle-unit')) {
            var classes = '';
